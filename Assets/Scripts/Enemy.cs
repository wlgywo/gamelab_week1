...
public class Enemy : MonoBehaviour
{
    ...
    protected virtual void FixedUpdate()
    {
        ...
        float distance = Vector3.Distance(transform.position, repairKit.position);
        if(attackCooldown < 0) {
            attackCooldown = attackDelay;
            Attack();
        } else {
            Vector3 dir = repairKit.transform.position - transform.position;
